...
RSpec.describe 'Trip_show', type: :system do
  ...
  describe "ページレイアウト" do
    context '全てのユーザー' do
      ...
      example '正しいユーザーinfoが表示されること' do
        within(".trip-show-first") do
          expect(page).to have_content user.user_name
        end
      end

      example '正しい旅行infoが表示されること' do
      ...
      example '正しいセクション名(スケジュール)が表示されること' do
        expect(page).to have_content "スケジュール"
      end

      example '正しいスケジュールが表示されること' do
  ...
  describe "編集・削除ボタン" do
    context '自分の旅行ページ' do
      ...
      example "旅行プランが削除できること", js: true do
        within(".trip-show-btn") do
          click_on "削除"
        end
